...
export const CookieConsent = () => {
  ...
  return (
    ...
    >
      ...
      {/* Periwinkle attention card */}
      <div
        className="relative w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl text-foreground p-6 sm:p-7 ring-1 ring-foreground/10 animate-in fade-in zoom-in-95 slide-in-from-bottom-4 duration-300"
        style={{
          background:
            "linear-gradient(135deg, hsl(219 100% 84%) 0%, hsl(230 75% 68%) 100%)",
          boxShadow:
            "0 25px 60px -15px hsl(230 75% 50% / 0.45), 0 10px 30px -10px hsl(230 22% 8% / 0.25)",
        }}
      >
        {/* Floating cookie chip */}
        <div className="absolute -top-5 left-6 sm:left-7 size-12 rounded-full bg-white text-foreground flex items-center justify-center shadow-lg ring-1 ring-black/5 -rotate-12">
        ...
        <div className="pt-4">
          ...
          <div className="mt-5 flex flex-col sm:flex-row sm:flex-wrap gap-2.5">
            <button
              onClick={() => handle("all")}
              className="inline-flex items-center justify-center rounded-full bg-foreground px-5 py-2.5 text-sm font-extrabold text-background shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 transition-all duration-150"
            >
              Shot, dunk away
            </button>
            <button
